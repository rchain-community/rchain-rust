...
- **C64 — store errors read as negatives across the peer-sync, proposal and gateway paths** (found
  2026-09-24 by the U14 sweep; the gateway site fixed, the rest triaged site by site). C63's class one
  ...
  **Triaged, with the reason each is not in this commit.** *Held*: `casper/src/engine/node_running.rs`'s
  ...
  `perf-tail`*: `lfs_block_requester.rs:86` (a dropped `block_store.put` followed by `guard.done(&hash)`:
  the block is recorded as done and never re-requested) and `:128` (a `contains` flatten whose empty
  `Vec<bool>` empties both `existing` and `missing`, so the walk requests nothing) — the two worst
  sites, folded into U13. *Owed, designed, needing a harness*: `proposer.rs:258`'s

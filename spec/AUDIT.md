...
| A free level a pattern declares and its matcher does not bind is **refused** by `RhoMatch::get` (through the `Match` trait's error channel), where the Scala pads the binding with the empty par | `rholang/interpreter/storage/package.scala:22-29` — `toSeq`'s `case None => Par.defaultInstance`; `Match.get` is `F[Option[A]]` (`Match.scala:11`), so the oracle had a channel for it and chose to pad | the count and the pattern disagreeing is what the padding hides: `free_count` is *defined* as the number of free levels the pattern binds, and `RhoMatch::get` fills the continuation's environment from the matcher's free map for `0..free_count`, so a level with no binding hands the continuation a variable bound to nothing. Refusing is the "no silent partiality" rule (`spec/TYPE-SYSTEM.md` §1.6) applied where the oracle flattens, and it is the same rule `resolve_match` (`reduce.rs:2009`) already applied on the deploy path. **Stricter than the oracle, not a hard fork:** the normalizer computes the count from the same pattern, so no term reaches it — only a hand-built or wire-carried `BindPattern` whose count disagrees with its patterns can, and AUDIT C52 refuses the negative half of that family at the proto boundary |
| A match case's declared free level that the matcher's free map does not carry is **refused** by `resolve_match`, where the Scala binds the continuation's variable to the empty par | `Reduce.scala:352` — `Range(0, freeCount).foldLeft(env)((acc, e) => acc.put(freeMap.getOrElse(e, Par())))` | `free_count` *is* "the number of distinct free levels the pattern binds" (`models/src/types.rs`'s `FreeCount`), so a count that outruns the matcher's map contradicts its own meaning: the Scala hands the continuation a variable bound to nothing, and the port used to do the same. `resolve_match` returns `Result` already, so the refusal costs no signature change and no invented value — the same rule `spec/TYPE-SYSTEM.md` §1.6 states ("a failure must be refused, never flattened to a default"), applied where the Scala flattens. **Not a hard fork:** the count and the pattern are produced together by the normalizer, so no deploy reaches the branch, and only a count read unvalidated from the wire could (AUDIT C52). Its sibling `RhoMatch::get` keeps the Scala's default because its channel is `Option` — see §20's C52 row |
...
- **C65 — the block store's side of LFS sync swallowed two failures the oracle propagates, so a
  ...
  **Fixed** by making the walk fallible end to end: `save_block`/`process_block`/`request_next` return
  ...
  receivers, so a transient store failure costs the attempt (and, with C66 fixed, the node stays in
...
- **C66 — a failed LFS sync still signalled the node out of syncing, so it could run on an
  ...
  **Fixed** by moving the signal behind the outcome (`notify_when_restored`): only `Ok` notifies, so a
  ...
  assertion. What this pins is the *decision*; that a failed attempt leaves the node in syncing *end to
  end* needs a `NodeSyncing` fixture with a failing store and a mock transport, which is recorded as
  the next unit's work rather than implied here.
...
- **C63 — the state-export path flattened store errors into "no state", and one of them into
  ...
  **The residues are closed by the outright conversion** (2026-09-24, U12). U11 left three: the
  ...
  `lfs_tuple_space_requester.rs:270,271,328` and `node_runtime.rs:1984` now make the "chunk done" claim
...
- **C66 — store errors read as negatives across the peer-sync, proposal and gateway paths** (found
  2026-09-24 by the U14 sweep; the gateway site fixed, the rest triaged site by site). *Numbered C66
  rather than C64: `C64` is the sync's pacing entry and `C65` the block store's two fixes, both landed
  by the performance walk while this was being written — this entry's first commit used C64 and was
  renumbered here.* C63's class one
...
| C66 a failed LFS sync still signalled the node out of syncing, so it could run on an incomplete DAG | 10 | **fixed (2026-09-24)**: the sync task notified `finished` — the only signal `node_launch` waits on to enter `NodeRunning` — *after* the outcome match, on failure as well as success, where the oracle sequences `finished.complete(())` after `parJoinUnbounded.compile.drain` and stores the approved block only after the state ("to restart requesting if interrupted with incomplete state"). Found while fixing C65, whose fix routes block-store failures into exactly this path, so the two had to land together. Now `notify_when_restored` signals on `Ok` only, falsified both directions by `a_failed_sync_does_not_signal_the_node_out_of_syncing` (with the unconditional notify restored the `Err` half fails); the end-to-end transition needs a `NodeSyncing` fixture with a failing store, named as the next unit's work |

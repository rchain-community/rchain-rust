...
| 31 | Rholang | Every BNFC term is accepted, modulo a data list of documented deviations | *(pending)* `Syntax` | *(pending)* | **open** |
| 32 | Rholang | Lexical determinism: comments, `_`-idents, `bundle0`, number forms, the operator spellings | *(pending)* `Lex` | *(pending)* | **open** |
| 33 | Rholang | `parse (print p) ≡ p` on `Par` (the C13 round-trip, stated) | *(pending)* `Syntax` | *(pending)* | **open** |
| 34 | Rholang | Normalization is a function; a *value* position (operand, condition, target, datum, element, pattern, name) is normalized against an empty `par` — only a statement continuation inherits | *(pending)* `Normalize` | *(pending)* | **open** (AUDIT C21's rule, to be stated) |
...
| 38 | Rholang | Silence is specified: an unmatched receive or `match` yields no reduction **and no error** | `Rchain/Silence.lean` — `ReduceP` (the only rule that consumes a datum carries `spatialMatches data pattern`), `takesStep`, `receiveParP`; `takesStep_iff_reduces` owed | `spec/conformance/silence.tsv` ← `Rchain/Corpus.lean` (`silenceCases_decide`, 6 cases) · `rholang/tests/lean_silence_corpus.rs` | **checked** (the rule is the law; the tie to the search is owed and named) |
...
| 43 | JSON | Each endpoint's serialized shape equals the schema's (camelCase fields, `[]` semantics, error precedence) | *(pending)* `Json` | *(pending)* | **open** |

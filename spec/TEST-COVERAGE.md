...
| `casper/src/gateway/mod.rs`'s `current_height` — a shard head that cannot be read is an `Err` and the phase is refused, where it used to anchor the deploy at `valid_after = 0` (born expired, reported nowhere) | A block-API read failure answered a *negative* (`0` = the most permissive anchor), so the phase deploy expired on arrival and the participant never learned why: the U14 sweep's class, headed by this site because its consequence is invisible downstream. The gateway is Rust-first (no Scala oracle); its own submit path refuses a phase it cannot build, which is the shape adopted (AUDIT C66) | `current_height_refuses_a_head_it_cannot_read` (its witnessing form was the pre-fix test `current_height_is_zero_when_the_shard_is_unknown_or_errors`, whose assertion passed on the defect), `current_height_is_zero_when_the_shard_is_unknown` |

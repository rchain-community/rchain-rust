...
**1462 `#[test]`/`#[tokio::test]` unit functions + 126 integration tests** across 13 crates, with **26
...
Raised five times: 73.68 ⇒ 71 (Stages 0–2), 79.69 ⇒ 77 (Stages 3–4: the legacy corpus, the tier sweep,
...
route in `node/tests/api_surface.rs`, and the first batches of item 11's ranking).
...
| 5. The coverage floor raised after measuring (five times) | **done** — 73.68 ⇒ 71, 79.69 ⇒ 77, 81.30 ⇒ 79, 84.07 ⇒ 82, 86.77 ⇒ 84. The floor is now *derived* rather than remembered: `tools/emit-coverage-ledger.sh` computes it from the measurement and check 11 refuses a CI floor that is not `floor(measured) − 2`. |

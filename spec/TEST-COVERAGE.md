...
| `rspace/src/match_.rs`, `space_matcher.rs`, `errors.rs` and the candidate-selection sites in `rspace.rs`/`replay_rspace.rs` — `Match::get` returns `Result<Option<A>, RSpaceError>` and the search functions are fallible with it, so a matcher failure is no longer read as "no match"; `RhoMatch::get` (`rholang/src/storage.rs`) refuses a free level the matcher did not bind instead of padding it | The oracle's `Match.get` is `F[Option[A]]` (`Match.scala:11`) and the Scala's `findMatchingDataCandidate`/`extractDataCandidates`/`extractFirstMatch` are in `F` too, so the port's bare `Option` flattened a channel that exists — two ways, by the end: a level the pattern declares and the matcher never binds became the empty par (the Scala's `toSeq`), and `fold_match(..).ok()?` turned a matcher `BugFoundError` into "no match", leaving a datum in the space for a bug's reason. **Stricter than the oracle**, which pads, so it is a §6 deviation (AUDIT C52) | `rho_match_refuses_a_free_count_its_pattern_does_not_bind` (its pre-fix form witnessed the defect — `pars.len() == 3` with two empty pars — and the post-fix form asserts the refusal), `find_matching_data_candidate_none_when_no_match` |

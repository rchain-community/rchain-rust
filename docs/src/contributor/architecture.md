...
**Corrected:** the `node` row above listed `revvaultexport` among the done features. It is **not ported** —
there is no Rust module and no CLI subcommand for it (the `Commands` enum has no `RevVaultExport`), and the
Scala oracle is `legacy/node/src/main/scala/coop/rchain/node/revvaultexport/` (seven files: the trie
traverser, the balance getter, the mainnet1 balance/reporting mains). It is a registered deferral in
`spec/RUST-FIRST.md`'s "Still deferred" sense rather than a done feature; a doc that claims otherwise is
the kind of claim this repository treats as a defect.

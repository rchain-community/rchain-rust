...
| [`spec/INVENTORY.md`](../../../spec/INVENTORY.md) | The **law catalog — 43 rows**: the 29 calculus laws plus 14 covering the surface a client writes (grammar, lexing, normalization, matching, reply shapes, JSON). One row per law, with source-of-truth, Rust realization, Lean/Coq target, status, and — for rows 30–43 — the corpus and the consumer. |

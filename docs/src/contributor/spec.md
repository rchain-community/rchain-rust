...
| [`spec/INVENTORY.md`](../../../spec/INVENTORY.md) | The **law catalog — 47 rows**: the 29 calculus laws, 14 covering the surface a client writes (grammar, lexing, normalization, matching, reply shapes, JSON), and 4 for the Proof-of-Stake epoch (the boundary gate, the reward split, its conservation, and withdrawal staging). One row per law, with source-of-truth, Rust realization, Lean/Coq target, status, and — for rows 30+ — the corpus and the consumer. |

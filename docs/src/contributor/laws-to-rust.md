...
| **29** | The coordinator's decision is a durable, content-addressed record; a prepared participant can always recover it; the merge/close record is re-derivable on replay. | the `PREFIX_TXN` native ledger (`rholang/src/native_state.rs`, `rspace/src/native_store.rs`); content-addressing analog of Laws 11/16 | `commit_record_deterministic` (`spec/Rchain/CrossShard.lean`) — stated |

...
| **28** | `prepare`/`commit`/`abort` are idempotent under the transaction id, so re-delivery and client retry are safe. | `txn_prepare`/`txn_commit`/`txn_abort` idempotent under `txn_id` (`rholang/src/native_state.rs`) | `leg_idempotent` (`spec/Rchain/CrossShard.lean`) — stated |

...
The **authoritative formal specification** is the [`spec/`](../../spec/) tree — the 22-law catalog
...
| Map a language feature to its **law** and its proof | [The 22 laws](formal/the-22-laws.md) |
...
RChain's behavior is pinned by **22 laws** (see [The 22 laws](formal/the-22-laws.md) and
...
|---|---|---|
| `spec/Rchain/*.lean` (Lean 4) | Law 1, `≡`/`⟶` core, `Closed`, totality fundamentals **proven**; Laws 3–5, 7–18 **stated**; Laws 20–22 **proven** in `Scheduler.lean` (except `law20_deadlock_freedom`, **stated**); crypto **axiomatized** | `cd spec && lake build` |
| `spec/coq/*.v` (Coq) | Laws 2–6 (substitution / α-equivalence metatheory) **stated** | `make -C spec/coq` |
| `spec/INVENTORY.md` | the 22-law catalog with source-of-truth + status | — |

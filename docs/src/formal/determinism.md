...
> the post-state hash of a block is a *pure function* of its inputs, founded in the 29 laws
...
> prior documents specify the components: [The 29 laws](the-29-laws.md), [The concurrency model](concurrency-model.md),
...
*one* transition function in both the play and replay paths. The 29 laws turn that into a theorem:
...
- **Genesis-vault re-seed** — `replay_block` passes no vaults; this *was* recorded here as "safe because
  genesis is trusted and never re-validated (an asserted invariant, not a code path)". **The
  parenthetical is false** (found 2026-09-23, on the 3-validator devnet): `casper/src/merging.rs:486-506`
  *does* replay the genesis. A node that did not create it has no mergeable-channel sidecar for it, so
  the sidecar is regenerated by replaying the block — and that replay passes `&[]` for the genesis
  wallet vaults, whose balances are `PREFIX_VAULT` leaves in the genesis post-state. The replay then
  computes a different hash and the validator refuses the block:
  `regenerated mergeable channels for block ae620156… but replay computed ad7be2fa… instead of
  052c997a…`, with the bootstrap proposing happily while validators 1 and 2 sit at the height they
  joined at. Reproduced in-process by
  `casper/tests/determinism.rs::a_genesis_replay_without_the_vaults_does_not_reproduce_the_genesis`,
  which asserts the divergence on purpose so that fixing the path fails it. The fix is not to pass the
  vaults unconditionally — a non-genesis block's pre-state already holds the post-genesis balances, so
  re-installing them would clobber them — but to re-install them when the block *is* the genesis
  (`pre_state_hash == empty_state_hash_fixed()`), which needs the genesis vault list reachable from the
  replay path. Recorded as AUDIT C46, with the devnet evidence.

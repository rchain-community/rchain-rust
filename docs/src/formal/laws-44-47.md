...
| 46 | the split **does not conserve**: `Σ rewards ≤ pot`, the difference being the dust of two integer divisions — which stays in the pot for the next epoch | `Rchain/Pos.lean` (`sum_rewards_le_pot`, `list_sum_div_le`, `div_add_div_le`, `the_dust_is_real`) | **proved over the model**, with a strict instance `decide`d: minimum bond 3, bonds `[4, 5]`, pot 10 — **six distributed of ten**. Written as an *equality* the claim would be false, and `Pos.rhox`'s own comment does not say which it means; the model settled it by computing the contract's arithmetic in `Nat` |

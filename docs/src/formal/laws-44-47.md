...
|---|---|---|---|
| 44 | membership takes effect at an **epoch boundary**: the sequence runs only when `blockNumber % epochLength = 0`; a bond is pooled but not activated, a withdrawal is staged but not moved, a claim is not paid | `rholang/src/native_state.rs`'s `close_block` gate (`is_epoch_boundary`) | **open** — implemented and pinned by `the_epoch_gate_does_nothing_off_a_boundary` (which builds the off-boundary state with a staged withdrawal *and* a full pot, then asserts nothing changed and the same call at the boundary moves and pays it), plus the two membership tests. No Lean model of the transition: `Rchain/Pos.lean` models the epoch's *arithmetic*, not its state machine |
...
| 47 | a withdrawal is **staged**: the request records `quarantineLength + epochLength * (1 + blockNumber / epochLength)` and changes nothing else; the validator leaves the pool at the next boundary and is paid `bond + committed rewards` at the first boundary past its quarantine | `rholang/src/native_state.rs` (`withdraw`, `close_block`'s move and pay steps) | **open** — pinned by `withdraw_stages_the_validator_until_the_next_boundary` (the three stages separately) and `a_released_withdrawal_pays_the_bond_plus_the_committed_rewards` (the sum, *and* that the staking vault is exactly emptied by it). No Lean model of the transition yet |

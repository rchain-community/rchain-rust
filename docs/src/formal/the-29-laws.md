...
> Its total is **34 of 49 laws proved at all** — eight tied to the node by a corpus, twenty-six over
...
|---|---|---|---|---|---|
| **1** | canonicalization is idempotent & commutative: `sort(sort p)=sort p`, `sort(p\|q)=sort(q\|p)` | the sorted `Par`, commutative `ESet`/`EMap` | `Sort.lean` — `sortPar_idempotent` / `sortPar_comm` (**proven**, mod the 12 element-comparator axioms the register counts — the ten element `lt_trans` laws plus `cmpExpr`'s `eq_iff` and `swap`; the list laws and `cmpGUnforgeable_lt_trans` are discharged) | `Sort.v` — `sortPar_idempotent` / `sortPar_comm` (axioms) | normalization (α + canonical `\|` sort) |
